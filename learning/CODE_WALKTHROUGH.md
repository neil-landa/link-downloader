...
```
**What this does:** Creates downloads folder
...
```
**What this does:** Builds the command as a list
- Each item is a separate argument
- Equivalent to: `yt-dlp --impersonate chrome -x --audio-format m4a -o "path" "url"`
...
```
**What this does:** Runs the command
- `capture_output=True`: Capture stdout/stderr (don't print to console)
- `text=True`: Return output as string (not bytes)
- `timeout=600`: Kill process after 10 minutes (prevents hanging)
...
```
**What this does:** Check if command succeeded
...
```
**What this does:** Defines the home page route
...
```javascript
      ...
      errorDiv.textContent = `Error: ${errorData.error || "Unknown error"}`;
      errorDiv.style.display = "block";
```
**What this does:** Handles errors
- `response.json()`: Parses JSON error message
- Sets error div text
- Shows error div (was hidden)
