...
# OR for Amazon Linux: sudo yum update -y

# Install Python 3.11
sudo apt install python3.11 python3.11-venv python3-pip -y  # Ubuntu
# OR for Amazon Linux: sudo yum install python3.11 python3.11-pip -y

# Install curl dependencies for yt-dlp impersonate feature
sudo apt install -y curl libcurl4-openssl-dev  # Ubuntu
# OR for Amazon Linux: sudo yum install curl libcurl-devel -y

# Install yt-dlp
pip3.11 install --upgrade yt-dlp

# Install Node.js for YouTube default player client (required for best compatibility)
curl -fsSL https://deb.nodesource.com/setup_20.x | sudo -E bash -
sudo apt install -y nodejs  # Ubuntu
# OR for Amazon Linux: Use different Node.js installation method

# Verify Node.js installation
node --version
npm --version

# Install Python dependencies (includes curl-cffi for impersonate)
pip3.11 install -r requirements.txt

# Install system dependencies for yt-dlp
sudo apt install ffmpeg -y  # Ubuntu
# OR for Amazon Linux: sudo yum install ffmpeg -y
```
...
Add this configuration (IMPORTANT: Static files must come BEFORE location /):
...
server {
    ...
    # Serve CSS files directly (faster than Flask)
    location /css/ {
    ...
    # Serve JS files
    location /js/ {
        alias /home/ubuntu/www/link-downloader/js/;
        expires 30d;
        add_header Cache-Control "public, immutable";
        access_log off;
    }
    ...
    # Serve image files
    location /img/ {
        alias /home/ubuntu/www/link-downloader/img/;
        expires 30d;
        add_header Cache-Control "public, immutable";
        access_log off;
    }
...
**Critical:** Static file locations (`/css/`, `/js/`, `/img/`) MUST come before `location /` or they'll be proxied to Flask instead of served directly.
...
#### 8. Set Up File Permissions for Nginx

Nginx runs as `www-data` user and needs to read your files:
...
sudo chmod -R 755 /home/ubuntu/www/link-downloader
sudo find /home/ubuntu/www/link-downloader -type f -exec chmod 644 {} \;
sudo find /home/ubuntu/www/link-downloader -type d -exec chmod 755 {} \;

# Verify nginx can read files
sudo -u www-data cat /home/ubuntu/www/link-downloader/css/style.css
```

#### 9. Set Up YouTube Cookies (Recommended)

YouTube requires authentication cookies to bypass bot detection:

```bash
# Export cookies from your browser (see COOKIES_SETUP.md)
# Upload to server:
...
Cookies expire every 2-4 weeks. Set up automated refresh (see `refresh-cookies-local.sh`).

#### 10. Set Up SSL with Let's Encrypt (Free HTTPS)
...
3. **Update HTML to Use S3 URLs:**
   ...
   <link
     rel="stylesheet"
     href="https://script-neillanda-static.s3.amazonaws.com/css/style.css"
   />
   ```
...
# Test manually
cd /home/ubuntu/www/link-downloader  # Ubuntu path
# OR cd /home/ec2-user/link-downloader  # Amazon Linux path
python3.11 app.py
```
...
# If cookies are expired, refresh them (see COOKIES_SETUP.md)
```
...
```bash
# Check nginx config order (static files must come first)
sudo nginx -t
